(function() {
     ...
     * Make a screenshot for each url in failure
     * Possibility to retry the check if document body match some regexp
     *
     * @params array   list
     * @params boolean checkNotice
     * @params RegExp  retryCondition
     * 
     * @TODO maybe a callback could be send as arguments, but this not the first aim of this method
     */
    CheckerToolkit.prototype.doStandardCheckFromUrlList = function (list, checkNotice, retryCondition) {

        if (typeof checkNotice === 'undefined') {
            checkNotice = false;
        }

        if (typeof retryCondition === 'undefined') {
            retryCondition = null;
        }
        ...
        var launchTestOnUrl = function () {
            ...
            if (!self.doStandardCheck(checkNotice, retryCondition)) {
                var captureFilename = casper.status().requestUrl.replace(':', '_').split('/').join('_');
                casper.capture(buildPath + '/' + captureFilename + '.png');
            }
        };
    ...
    /**
     * Check for HTTP and PHP errors
     *
     * @return CheckerToolkit
     */
    CheckerToolkit.prototype.doStandardCheck = function (checkNotice, retryCondition) {
        if (typeof checkNotice === 'undefined') {
            checkNotice = false;
        }

        var self   = this,
            errPhp = this.casper.evaluate(this.getPhpErrors, checkNotice),
        ...
        if ((this.retryNumber < this.maxRetryNumber) && typeof retryCondition !== 'undefined' && retryCondition) {
            retry = this.casper.evaluate(function (retryCondition) {
                if (document.body && document.body.innerHTML.match(retryCondition)) {
                    return true;
                }
            ...
            if (retry) {
                this.test.info('*** Condition for retesting url is true - Document body matchs RegExp ' + retryCondition.toString() + ' ***');
                this.retryNumber++;
                this.open(this.casper.status().requestUrl, function () {
                    self.doStandardCheck(checkNotice, retryCondition);
                });
            } else {
     ...
     */
    CheckerToolkit.prototype.checkRedirect = function(url, timeout) {
        ...
        var test = this.test;
        this.casper.waitForUrl(url, function() {
            test.pass('Redirected to ' + url);
        }, function() {
            test.fail('URL ' + url + ' expected');
        }, timeout);
    };
     ...
     * @return array res
     */
    CheckerToolkit.getPhpErrors = function (checkNotice) {
        ...
        if (document.body) {
            ...
            if (checkNotice && document.body.innerHTML.indexOf('Notice') !== -1) {
