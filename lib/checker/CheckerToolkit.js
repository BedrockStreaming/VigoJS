(function() {
     ...
     */
    CheckerToolkit.prototype.doStandardCheckFromUrlList = function (list, checkNotice) {
        ...
        var launchTestOnUrl = function () {
            var phpError = casper.evaluate(currentCheck.getPhpErrors, checkNotice),
                errBefore,
                errAfter,
                captureFilename;

            errBefore = test.suiteResults.getAllFailures();
            currentCheck.checkStatus();
            currentCheck.checkPhpErrors(phpError);
            errAfter = test.suiteResults.getAllFailures();

            if (errAfter.length > errBefore.length) {
                captureFilename = casper.status().requestUrl.replace(':', '_').split('/').join('_');
